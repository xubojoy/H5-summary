...
<div id="main" class="bg_f4">
	...
	<ul class="orderPersonInfo w100p15 bg_white" style="margin-top:10px" >
		...
		<li>
			<span class="font15 font_gray">预约理疗院名</span>
			<span class="font16 font_dark_gray" style="padding-right:3px;float: right;width: -moz-calc(100% - 91px);width:-webkit-calc(100% - 91x);width:calc(100% - 91px)">${order.organization.name }</span>
		</li>
		<li>
			<span class="font15 font_gray">预约理疗项目</span>
			<span class="font16 font_dark_gray" style="padding-right:3px;float: right;width: -moz-calc(100% - 91px);width:-webkit-calc(100% - 91x);width:calc(100% - 91px)">${order.project.name }</span>
		</li>
		<li>
	...
	<c:choose>
		...
		<c:when test="${order.orderStatus eq 'completed' && order.commentFlag eq false }">
			<div class="footerBtn bg_white" onclick="evaluateOrder('${order.id}');">
				<span class="orderBtn font18 font_white bg_dark_brown">评价</span>	
			</div>
		</c:when>
	</c:choose>
</div>
